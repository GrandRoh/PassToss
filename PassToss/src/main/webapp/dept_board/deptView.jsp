...
<script>
	$(function(){
		$("#button").click(function(){
			location.href="DeptWrite.bod";
		})
	});
</script>
<style>
	/*table-striped 글 목록에 회색색깔 나오게 하는거 다시 넣고 싶으면 table에 넣으면 됨 */
...
<body>
	...
	<input type="hidden" id= "deptno" value="${deptno}" name="deptno">
	  <form action="DeptList.bod" method="post">
	   		 	...
	   		 	<tr>
	   		 	  ...
	   		 	  <td> <%-- 제목 --%>
	   		 	    <div>
	   		      	...
	   		      	<a href="DeptDetailAction.bod?num=${b.board_num}">
		 ...
		 <c:if test="${listcount > 0}">
		  <div class="center-block">
	  	 	<ul class="pagination justify-content-center">
	  	 		...
	  	 		<c:if test="${page > 1}">
	  	 			<c:url var="prev" value="DeptList.bod">
	  	 		...
	  	 		<c:forEach var="a" begin="${startpage}" end="${endpage}">
	  	 	 	 <c:if test="${a == page}">
	  	 	 		 ...
	  	 	 		 <c:if test="${a != page}">
	  	 	  			<c:url var="go" value="DeptList.bod">
	  	 	  			<c:param name="search_field" value="${search_field}" />
	  	 	  			<c:param name="search_word" value="${search_word}" />
	  	 		...
	  	 		<c:if test="${page < maxpage}">
	  	 			<c:url var="next" value="DeptList.bod">
	  	 	  		<c:param name="search_field" value="${search_field}" />
	  	 	  		<c:param name="search_word" value="${search_word}" />
	  	 	  		<c:param name="page" 	value="${page+1}" />
	  	 	  	</c:url>
		...
		<span><button id="button" type="button" class="write">글 쓰 기</button></span>
