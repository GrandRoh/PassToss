...
public class CommentfreeDAO {
	...
	public JsonArray getCommentList(int comment_board_num, int state) {
		...
		String sql = "select num, comment_free.id, content, comment_date, comment_re_lev, "
				   + "comment_re_seq, comment_re_ref, member.profileImg "
				   + "from comment_free join member "
				   + "on comment_free.id = member.id "
				   + "where comment_board_num = ? "
	...
	public int commentsUpdate(Commentfree co) {
